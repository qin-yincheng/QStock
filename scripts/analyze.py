...
用法：python scripts/analyze.py --symbol 600519.SH
      python scripts/analyze.py --symbol 600519.SH --freq F60
"""
...
def format_report(result: dict) -> str:
    ...
    lines = [
        ...
        f"  分析时间: {dt}",
        f"  最新价格: {close:.2f}",
        f"  K线数量: {bar_count} 根（{result.get('freq', '30分钟')}级别）",
        "",
        "  【综合信号】",
...
def main():
    ...
    parser.add_argument("--sdt", default=None, help="开始日期 YYYYMMDD（默认180天前）")
    ...
    try:
        result = analyze_stock_realtime(symbol, freq=freq)
        print(format_report(result))
    except Exception as e:
        print(f"[WARN] 实时分析失败：{str(e)[:60]}，使用历史数据...")
        result = analyze_stock(symbol, sdt, edt, freq=freq)
        print(format_report(result))
